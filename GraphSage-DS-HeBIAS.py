...
def train(epoch):
    ...
    teacher_output = teacher_output['author']/ args.temperature
    out = model(graph.x_dict, graph.edge_index_dict)
    out = out['author']
    _, pred = (out.max(dim=1))        
    focalloss = focal_loss(out[train_mask], graph['author'].y[train_mask])
    label_f1_scores ,F1_test ,F1_bias ,F1_mean= calculate_f1_score(pred, y, train_mask)       
    loss = args.beta * loss_function(out[train_mask], graph['author'].y[train_mask])    
...
for epoch in tqdm(range(150)):
    ...
    Tea_model = GraphSage(-1, 128, num_classes)
