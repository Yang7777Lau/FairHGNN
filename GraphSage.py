...
from torch_geometric.nn import HANConv,GCNConv,to_hetero,SAGEConv
import argparse
...
def train():
    model = GraphSage(-1, 128, num_classes)
    ...
    for epoch in tqdm(range(150)):
        ...
        loss = loss_function(out['paper'][train_mask], graph['paper'].y[train_mask])
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        max_f1 = max(label_f1_scores.values())
        min_f1 = min(label_f1_scores.values())
        print(max_f1-min_f1)
        '''
...
def test(model, mask):
    ...
    _, pred = out['paper'].max(dim=1)

    correct = int(pred[mask].eq(graph['paper'].y[mask]).sum().item())
    acc = correct / int(mask.sum())
    label_accuracy = calculate_label_accuracy(pred, y, mask)
    loss_function = torch.nn.CrossEntropyLoss().to(device)
    loss = loss_function(out['paper'][mask], graph['paper'].y[mask])
...
print('GraphSage F1-sorce:', "%0.2f%%" % (final_F1 * 100))
print('GraphSage F1-mean:', "%0.2f%%" % (final_F1_mean * 100))
print('GraphSage F1-bias:', "%0.2f%%" % (final_F1_bias * 100))