...
class HAN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        ...
        self.conv1 = HANConv(in_channels, hidden_channels, graph.metadata(), heads=1)
        self.conv2 = HANConv(in_channels, hidden_channels, graph.metadata(), heads=1)
...
def train(epoch):
    ...
    loss += args.alhpa * KLloss(F.log_softmax(out[train_mask], dim=-1),
                                  F.log_softmax(teacher_output[train_mask], dim=-1))
...
print('HAN F1-sorce-HeBias:', "%0.2f%%" % (final_F1 * 100))
print('HAN F1-mean-HeBias:', "%0.2f%%" % (final_F1_mean * 100))
print('HAN F1-bias-HeBias:', "%0.2f%%" % (final_F1_bias * 100))