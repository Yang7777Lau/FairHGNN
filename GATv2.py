...
from torch_geometric.nn import HANConv,GCNConv,to_hetero,SAGEConv ,GraphConv ,GCNConv ,GATv2Conv
import argparse
...
def train():
    ...
    for epoch in tqdm(range(150)):
        ...
        loss = loss_function(out['movie'][train_mask], graph['movie'].y[train_mask])
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def test(model, mask):
    ...
    _, pred = out['movie'].max(dim=1)

    correct = int(pred[mask].eq(graph['movie'].y[mask]).sum().item())
    acc = correct / int(mask.sum())
    label_accuracy = calculate_label_accuracy(pred, y, mask)
    loss_function = torch.nn.CrossEntropyLoss().to(device)
    loss = loss_function(out['movie'][mask], graph['movie'].y[mask])
