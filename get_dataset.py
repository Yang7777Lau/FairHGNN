...
from torch_geometric.utils import degree,to_dense_adj,to_undirected
...
from GraphENS_utils import saliency_mixup, compute_saliency
from oversample_utils import oversample,oversample_homo
from Graphsmote_utils import smote_and_generate_edges
from utils import Homo_Hype
parser = argparse.ArgumentParser(description='dataset')
parser.add_argument('--seed', type=int, default=1)
parser.add_argument('--device', type=int, default=0)
parser.add_argument('--delta', type=int, default=0.5)
...
def get_dataset(root, dataname, transform=NormalizeFeatures(), train_mask_ratio=0.6, val_mask_ratio=0.2):
    ...
    if dataname == 'ACM':
        ...
        graph = ACM_smote_and_generate_edges(graph, 0, 1)
        #graph = oversample( graph ,'paper', 0, 1 )
    ...
    if name =='DBLP_processed' :
        ...
        print(graph)
        graph = DBLP_smote_and_generate_edges(graph, 1, 1 )
        #graph = oversample( graph ,'author', 1, 1 )
    ...
    if name =='IMDB':
        ...
        graph = IMDB_smote_and_generate_edges(graph, 0,1 )
        #graph = oversample( graph ,'movie', 0, 1 )
...
def poly_focal_loss(alpha, inputs, targets, gamma=2., epsilon=0.2):#γ=2

    alpha = alpha[targets]
    ce_loss = F.cross_entropy(inputs, targets, reduction='none')  
    pt = torch.exp(-ce_loss)  
    focal_loss = alpha * (1 - pt) ** gamma * ce_loss
    Poly = focal_loss + epsilon * (1 - pt) ** (gamma + 1)
    return torch.mean(Poly)


def IMDB_smote_and_generate_edges(graph, nu, num_synthetic_samples):

    minority_indices = torch.where(graph['movie'].y == nu)[0]
    minority_indices = torch.randperm(len(minority_indices))[:int(1135*1)] #215:0.2

    features = graph['movie'].x
    ...
    for idx in minority_indices:
        ...
        nbrs = NearestNeighbors(n_neighbors=num_synthetic_samples + 1, algorithm='auto').fit(features.cpu().numpy())
        distances, indices = nbrs.kneighbors(features[idx].cpu().numpy().reshape(1, -1))

        nearest_neighbors = torch.from_numpy(indices[:, 1:]).squeeze(0).to(features.device)
        

        synthetic_nodes = features[nearest_neighbors] * args.delta + (1 - args.delta) * features[idx].unsqueeze(0)

        
        new_nodes.append(synthetic_nodes)
    
    

    new_nodes = torch.stack(new_nodes, dim=0)
    new_nodes = new_nodes.squeeze(1)
    ...
    for i, idx in enumerate(minority_indices):
        ...
        new_edges = torch.cat((new_edges, torch.tensor([[idx], [i + graph['movie'].num_nodes - num_new_nodes]])), dim=1)
 
        for neighbor_idx in torch.from_numpy(indices[:, 1:]).squeeze(0):
            new_edges = torch.cat((new_edges, torch.tensor([[neighbor_idx], [i + graph['movie'].num_nodes - num_new_nodes]])), dim=1)
    

    graph['movie', 'to', 'movie'].edge_index = new_edges
    
    
    return graph

def DBLP_smote_and_generate_edges(graph, nu, num_synthetic_samples):
    
    minority_indices = torch.where(graph['author'].y == nu)[0]
    minority_indices = torch.randperm(len(minority_indices))[:745]

    features = graph['author'].x
    ...
    for i, idx in enumerate(minority_indices):
        new_edges = torch.cat((new_edges, torch.tensor([[idx], [i + graph['author'].num_nodes - num_new_nodes]])), dim=1)
        for neighbor_idx in torch.from_numpy(indices[:, 1:]).squeeze(0):
            new_edges = torch.cat((new_edges, torch.tensor([[neighbor_idx], [i + graph['author'].num_nodes - num_new_nodes]])), dim=1)
    
    graph['author', 'to', 'author'].edge_index = new_edges
    
    
    return graph

def ACM_smote_and_generate_edges(graph, nu, num_synthetic_samples):

    minority_indices = torch.where(graph['paper'].y == nu)[0]
    minority_indices = torch.randperm(len(minority_indices))[:int(1061*1)]
    ...
    for i, idx in enumerate(minority_indices):
        ...
        new_edges = torch.cat((new_edges, torch.tensor([[idx], [i + graph['paper'].num_nodes - num_new_nodes]])), dim=1)

        for neighbor_idx in torch.from_numpy(indices[:, 1:]).squeeze(0):
            new_edges = torch.cat((new_edges, torch.tensor([[neighbor_idx], [i + graph['paper'].num_nodes - num_new_nodes]])), dim=1)
    

    graph['paper', 'to', 'paper'].edge_index = new_edges
    
    
    return graph


def get_homo_dataset(root, dataname, transform = NormalizeFeatures(), train_mask_ratio=0.2, val_mask_ratio=0.2, test_mask_ratio = 0.6):
    pyg_dataset_dict = {
    ...
    #graph = smote_and_generate_edges(graph,0,1)
    
    num_nodes = graph.num_nodes
    num_classes = torch.max(graph.y).item() + 1

    num_train = int(train_mask_ratio * num_nodes)
    num_val = int(val_mask_ratio * num_nodes)
    num_test = int(test_mask_ratio * num_nodes)
    ...
    train_label_counts = torch.bincount(graph.y[graph.train_mask])
    val_label_counts = torch.bincount(graph.y[graph.val_mask])
    test_label_counts = torch.bincount(graph.y[graph.test_mask])
    
    if dataname == 'PubMed' or dataname == 'Cora':
        graph.edge_index = to_undirected(graph.edge_index)
    
    for label in range(num_classes):
        print("Label {}: Train Nodes: {}, Val Nodes: {}, Test Nodes: {}".format(label, train_label_counts[label], val_label_counts[label], test_label_counts[label]))
        label_counts = torch.bincount(graph.y)
    print(graph)    
    return G, graph, num_classes