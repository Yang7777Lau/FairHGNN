...
from torch_geometric.nn import HANConv ,SAGEConv ,to_hetero ,GraphConv ,GATConv
import argparse
...
parser.add_argument('--alhpa', type=float, default=0.2)
...
def train(epoch):
    ...
    teacher_output = teacher_output['movie']/ args.temperature
    out = model(graph.x_dict, graph.edge_index_dict)
    out = out['movie']
    _, pred = (out.max(dim=1))        
    focalloss = focal_loss(out[train_mask], graph['movie'].y[train_mask])
    label_f1_scores ,F1_test ,F1_bias ,F1_mean= calculate_f1_score(pred, y, train_mask)       
    loss = args.beta * loss_function(out[train_mask], graph['movie'].y[train_mask])    
...
for epoch in tqdm(range(150)):
    ...
    Tea_model = GNN(-1, 128, num_classes)
...
print('GAT F1-sorce-HeBias:', "%0.2f%%" % (final_F1 * 100))
print('GAT F1-mean-HeBias:', "%0.2f%%" % (final_F1_mean * 100))
print('GAT F1-bias-HeBias:', "%0.2f%%" % (final_F1_bias * 100))